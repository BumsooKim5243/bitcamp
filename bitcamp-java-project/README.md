...
- 작업내용
    - 팀, 회원, 게시물 정보 입력/변경할때 날짜 값을 Date 객체에 저장한다.
...
- 학습목표
    ...
    - 생성자 문법의 존재 의미를 이해한다.
- 작업내용
    - XxxController객체를 생성할 때 필수 값으로 Scanner 객체를 받도록 변경한다.

## src11 - 인스턴스 변수와 인스토리 메서드의 활용(리팩토링)
- 학습목표
    - 인스턴스 변수의 사용법을 익힌다.
    - 인스턴스 메서드의 사용법을 익힌다.
- 작업내용
    - 스택틱 변수와 스태틱 메서드로 만든 기존 코드를 인스턴스 변수와 인스턴스 메서드로 변경한다.
...
- 작업내용
  - App 클래스에 사용자에 입력한 메뉴에 따라 컨트롤러의 각 메서드를 호출하는
  코드를 컨트롤러에게 옮긴다.
...
계속 입력하시겠습니가?(Y/n) n
----------------------------
비트비트1, 5명, 2018-05-05 ~ 2018-08-20
